...
private:
     ...
     ***********************************************************************/
    friend std::ostream &operator<<(std::ostream &dataOut, intelhex &ihLocal);
     ...
     ***********************************************************************/
    friend std::istream &operator>>(std::istream &dataIn, intelhex &ihLocal);
...
private:
     ...
     ***********************************************************************/
    void decodeDataRecord(std::uint8_t recordLength, std::uint32_t loadOffset, std::string::const_iterator data);
...
public:
    ...
    bool endOfData()
    {
        ...
        if (!ihContent.empty()) {
            return --(ihContent.end()) == ihIterator;
        } else {
            return true;
        }
    }
    ...
    bool jumpTo(std::uint32_t address)
    {
        ...
        if (ihContent.size()) {
            auto it = ihContent.find(address);
            if (it != ihContent.end()) {
                result = true;
                ihIterator = it;
            }
    ...
    {
        if (ihContent.size()) {
            *address = ihContent.rbegin()->first;
            return true;
        } else {
            return false;
        }
     ...
     ***********************************************************************/
    bool getStartSegmentAddress(std::uint16_t *ipRegister, std::uint16_t *csRegister)
    {
     ...
     *     setStartLinearAddress()
     ***********************************************************************/
    void setStartSegmentAddress(std::uint16_t ipRegister, std::uint16_t csRegister)
    {
